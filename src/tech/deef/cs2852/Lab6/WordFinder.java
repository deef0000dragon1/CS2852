...
import tech.deef.cs2852.Lab6.GamePiece;
import tech.deef.cs2852.Lab6.UI;

/**
 * This is the powerhouse of the application. This is where all the recursing
 * and calculating takes place. This class also contains methods to recurse
 * through, access, and manipulate the search grid of GridEntries.
 * 
...
public class WordFinder {
	 ...
	 * 
	 * @param gameBoard
	 *            The game board to be searched
	 * @param dictionary
	 *            A collection of correctly spelled words
	 * @param rows
	...
	public WordFinder(List<GamePiece<Character>> gameBoard, Dictionary dictionary, int rows, int cols) {
		...
		if(dictionary.Contains("riey")){
			System.out.println("contsins****************8");
		}
	}
	 ...
	 * through the game board using the specified search pattern, creating every
	 * possibly letter combination and checking it against a dictionary. If the
	 * word is found in the dictionary, it gets added to a collection of found
	 * words.
	 * 
	 * @param row
	 *            Current row position of cursor
	 * @param col
	 *            Current column position of cursor
	 * @param do8WaySearch
	 *            <tt>true</tt> if eight way search is desired, otherwise a four
	 *            way search is performed
	 */
	private void recursiveSearch(int row, int col, boolean do8WaySearch) {
		...
		//System.out.println(row + "\t" + col + "\t" + currWord);
		if (row >= rows || row < 0 || col >= cols || col < 0 || currWord.length() >= MAX_WORD_LENGTH
				|| gameBoard.get(row * cols + col).hasBeenVisited()) {
			return;
		}
		...
		// 2. Now we're on a game piece we haven't yet visited on this pass
		// through.
		// Grab the character on the game piece and append it to the current
		// word
		// we're building.
		currWord += gameBoard.get(row * cols + col).getElement();
	...
	/**
	 * Removes the visited flag from the game piece at row, col on the board.
	 * 
	 * @param row
	 *            Row in which the game piece is located
	 * @param col
	 *            Column in which the game piece is located
	 */
	 ...
	 * 
	 * @param word
	 *            The word to be recorded as valid
	 */
	...
	public void startSearching() {
		...
		for (int r = 0; r < rows; ++r) {
			for (int c = 0; c < cols; ++c) {
				if (!GUI_ENABLED) {
					System.out
							.print("Searching from " + getGamePiece(r, c).getElement() + " (" + r + ", " + c + ")... ");
				}
				...
				if (!GUI_ENABLED) {
					System.out.println(time.format(100.0 * (r * cols + c + 1) / gameBoard.size()) + "% done!");
				}
			}
		}
		if (!GUI_ENABLED) {
			...
			System.out.println("\nTotal recursion time: " + time.format(hours) + ":" + time.format(minutes) + ":"
					+ timeS.format(seconds));
		}
