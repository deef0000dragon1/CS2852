...
/** 
 * clas in charge of starting and mantaining the program. initalizes the gui and all buttons. 
 * Waits for input. from user after.  
 * */
public class DotViewer extends JFrame {

	private static final long serialVersionUID = 1L;
	public List<Dot> original = new ArrayList<Dot>();
	public ArrayList<Dot> changed = new ArrayList<Dot>();
	...
	public void loadImage() {
		...
		int returnVal = chooser.showOpenDialog(this); //gets the file using a file chooser
		//lists must be initalized once for addition of points to work.
		if (returnVal == JFileChooser.APPROVE_OPTION) {
			original = CreatePoints.ReadFromFile(new File(chooser.getSelectedFile().getPath())); //creates the list of original points
		}

		plotter = new WinPlotter(); //SHOULD erase the current plotter and redraw it. Does not. 
		plotter.setPenColor(Color.BLACK.getRed(), Color.BLACK.getGreen(), Color.BLACK.getBlue()); 
		plotter.setBackgroundColor(Color.WHITE.getRed(), Color.WHITE.getGreen(), Color.WHITE.getBlue()); //sets the colors of the pen/background
		plotter.setWindowSize(600, 600);//sets the window size
		// plotter.setPlotBoundaries(getMinX()-1, getMinY()-1, getMaxX()+1,
		// getMaxY()+1);
		plotter.setPlotBoundaries(0, 0, 1, 1); //fudged plot boundries code to account for above code not working.
		plotter.moveTo(original.get(original.size() - 1).getX(), original.get(original.size() - 1).getY()); //moves the plotter to the last point
		for (int i = 0; i < original.size(); i++) {
			plotter.drawTo(original.get(i).getX(), original.get(i).getY()); //draw loop that draws from last point through all points to end point
		}
		numberOfPoints = original.size() + ""; //sets the number of points string to the number of points currently. 
	}
	...
	/** 
	 * builds the gui and creates initalizes changed and original.
	 * Unknown why  extra initalization code is necessary. 
	 * */
	public DotViewer() {
		super();
		buildGUI();
	...
	private void buildGUI() {
		...
		SelectButton.addActionListener(e -> {
			plotter = null; //sets the plotter to null. unknown why this is necessary.
			loadImage();//load image function
			numberOfPointsLabel.setText("Points In Original File: " + numberOfPoints); //change the text of the set text label
		});
		...
		DotButton.addActionListener(e -> {
			if (plotter != null) {//if the plotter exists
				CreatePoints.getDesiredDots(original, changed, Integer.parseInt(InputText.getText())); //update the dots to the number watted
				plotter.erase();//clear the plotter

				for (int i = 0; i < changed.size(); i++) {
					plotter.drawPoint(changed.get(i).getX(), changed.get(i).getY());//draw the points for each dot in the changed points
				}
		...
		LineButton.addActionListener(e -> {
			if (plotter != null) {
				...
				plotter.moveTo(changed.get(changed.size() - 1).getX(), changed.get(changed.size() - 1).getY()); //move the plotter tothe last dots position
				for (int i = 0; i < changed.size(); i++) {
					plotter.drawTo(changed.get(i).getX(), changed.get(i).getY()); //move to each point on the list nd draw the lines from the point to the next. 
				}
		...
		BothButton.addActionListener(e -> {
			if (plotter != null) {//does oth loops from points and from lines
				plotter.erase();
				CreatePoints.getDesiredDots(original, changed, Integer.parseInt(InputText.getText()));
		...
		layout.setConstraints(BothButton, makeGridBagLayoutConstraints(1, 5, 3, 1, GridBagConstraints.HORIZONTAL));
		add(BothButton);
